...
- 게시판 API의 `X-XSRF-TOKEN` 헤더는 선택 사항입니다.
...
회원가입 후에는 별도 로그인이 필요합니다. 일반 로그인은 기존 세션이 있으면 유지합니다.
...
제목은 1–200자, 본문은 1–10,000자이며 공백만 입력할 수 없습니다.
...
- 수정 시각 또는 첨부파일 목록이 변경된 경우 `409`를 반환합니다.

JSON 수정은 `title`, `content`만 전송합니다. 첨부파일 정보는 파일 목록 API에서 조회합니다.
...
댓글이 경로의 게시글에 속하지 않거나 게시글이 없으면 `404`를 반환합니다.
...
조회 대상은 `health`, `info`, `metrics`, `mappings`이며, health 구성 요소와 개별 지표를 포함해 GET·HEAD를 지원합니다. 변경 요청은 허용하지 않습니다.

`GET /actuator/health`는 정상일 때 `200`과 `status: UP`, DB·디스크 등의 상태 정보를 반환합니다. `DOWN`·`OUT_OF_SERVICE` 상태는 `503`을 반환합니다.
